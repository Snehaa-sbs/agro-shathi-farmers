...
const SignUp = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const phoneWithPrefix = form.phone.startsWith('+88') ? form.phone : `+88${form.phone}`;
    const { error } = await signUp(form.email, form.password, form.fullName, phoneWithPrefix);
    setLoading(false);

    if (error) {
      toast.error(error.message, { duration: 5000 });
    } else {
      toast.success(t('auth.signupSuccess'), { duration: 5000 });
      navigate('/');
    }
  };
  ...
  const handleGoogle = async () => {
    ...
    if (error) toast.error('Google sign in failed', { duration: 5000 });
  };
